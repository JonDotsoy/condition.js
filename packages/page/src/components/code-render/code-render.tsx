...
const selectAtom = (key: string, initialValue: string) => {
  ...
  const initValue: string =
    (globalThis.localStorage?.getItem(key) as any) ?? initialValue;
...
const DisplayTypeCodes: Record<string, string | undefined> = {
  ts: "typescript",
  js: "javascript",
};

export const CodeRender: React.FC<{
  payload: any;
  displayTypes?: string[];
    ...
    <>
      ...
      <pre
        ...
        dangerouslySetInnerHTML={{
          __html: hljs.highlight(render(), {
            language: DisplayTypeCodes[displayType] ?? displayType,
          }).value,
        }}
      ></pre>
    </>
